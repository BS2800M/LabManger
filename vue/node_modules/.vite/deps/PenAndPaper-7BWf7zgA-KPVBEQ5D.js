...
// node_modules/vue-data-ui/dist/PenAndPaper-7BWf7zgA.js
var ye = {
...
var Be = {
  ...
  setup(d, { emit: se }) {
    ...
    function O(l) {
      ...
      a.setAttribute("x", e), a.setAttribute("dy", "0"), a.textContent = "", t.appendChild(a), t.style.pointerEvents = "none", t.style.userSelect = "none", o.value.appendChild(t), f.value = t, Q.value = true, window.addEventListener("keydown", j), window.addEventListener("mousedown", _, true), J(), Z();
    }
    function j(l) {
      if (!Q.value) return;
      let { row: e, col: n } = D.value, t = N.value.slice(), a = false;
      if (l.key === "Enter") {
        const u2 = t[e], r = u2.slice(0, n), v = u2.slice(n);
        t.splice(e, 1, r, v), e += 1, n = 0, a = true, l.preventDefault();
      } else if (l.key === "Backspace") {
        if (n > 0)
          t[e] = t[e].slice(0, n - 1) + t[e].slice(n), n -= 1, a = true;
        else if (e > 0) {
          const u2 = t[e - 1].length;
          t[e - 1] += t[e], t.splice(e, 1), e -= 1, n = u2, a = true;
        }
        l.preventDefault();
      } else if (l.key === "Delete")
        n < t[e].length ? (t[e] = t[e].slice(0, n) + t[e].slice(n + 1), a = true) : e < t.length - 1 && (t[e] += t[e + 1], t.splice(e + 1, 1), a = true), l.preventDefault();
      else if (l.key === "ArrowLeft")
        n > 0 ? n -= 1 : e > 0 && (e -= 1, n = t[e].length), a = true, l.preventDefault();
      else if (l.key === "ArrowRight")
        n < t[e].length ? n += 1 : e < t.length - 1 && (e += 1, n = 0), a = true, l.preventDefault();
      else if (l.key === "ArrowUp")
        e > 0 && (e -= 1, n = Math.min(n, t[e].length), a = true), l.preventDefault();
      else if (l.key === "ArrowDown")
        e < t.length - 1 && (e += 1, n = Math.min(n, t[e].length), a = true), l.preventDefault();
      else if (l.key.length === 1 && !l.ctrlKey && !l.metaKey && !l.altKey)
        t[e] = t[e].slice(0, n) + l.key + t[e].slice(n), n += 1, a = true, l.preventDefault();
    ...
    function Z() {
      ...
      const e = f.value;
      if (!e) return;
      const { x: n, y: t } = H.value, { row: a, col: u2 } = D.value, r = L.value * s.scale, v = e.childNodes[a];
      if (!v) return;
      let g = v.textContent.slice(0, u2);
      g.endsWith(" ") && (g += " ");
    ...
    function _(l) {
      if (f.value && !f.value.contains(l.target)) {
        ...
        e.length === 1 && (e[0].textContent === "" || e[0].textContent === "​") && f.value.remove(), ee(false);
      }
    }
    function ee(l = false) {
      window.removeEventListener("keydown", j), window.removeEventListener("mousedown", _, true);
      ...
      l || t ? f.value && o.value.contains(f.value) && o.value.removeChild(f.value) : f.value && o.value.contains(f.value) && b.value.push(f.value), Q.value = false, f.value = null, N.value = [""], D.value = { row: 0, col: 0 };
    }
    const y = computed(() => Rt(s.color, 0.6));
    function te() {
      if (!o.value) return;
    ...
    function re(l) {
      ...
      const u2 = t[t.length - 2], r = t[t.length - 1];
      return a.push(`L ${u2} ${r}`), a.join(" ");
    }
    function ie(l, e = 1) {
      const n = [...l];
      for (let t = 2; t < l.length - 2; t += 2) {
        const a = l[t], u2 = l[t + 1], r = l[t - 2], v = l[t - 1], g = l[t + 2], i = l[t + 3];
        n[t] = a + e * ((r + g) / 2 - a), n[t + 1] = u2 + e * ((v + i) / 2 - u2);
      }
      return n;
    }
    function ve(l) {
      const e = l.trim().split(/\s+/);
      let n = "", t = "", a = null, u2 = null;
      for (let r = 0; r < e.length; r += 1) {
        const v = e[r];
        if (isNaN(v)) {
          if (t = v, t === "M" || t === "L")
            a = parseFloat(e[++r]), u2 = parseFloat(e[++r]), n += `${t}${a} ${u2}`;
          else if (t === "Q") {
            const g = parseFloat(e[++r]), i = parseFloat(e[++r]), p = parseFloat(e[++r]), m = parseFloat(e[++r]);
            g === a && i === u2 ? n += `t${p - a} ${m - u2}` : n += `q${g - a} ${i - u2} ${p - a} ${m - u2}`, a = p, u2 = m;
          }
        } else {
          const g = parseFloat(v), i = parseFloat(e[++r]);
          if (t === "L") {
            const p = g - a, m = i - u2;
            p === 0 ? n += `v${m}` : m === 0 ? n += `h${p}` : n += `l${p} ${m}`, a = g, u2 = i;
          } else if (t === "Q") {
            const p = g, m = i, C = parseFloat(e[++r]), w = parseFloat(e[++r]);
            p === a && m === u2 ? n += `t${C - a} ${w - u2}` : n += `q${p - a} ${m - u2} ${C - a} ${w - u2}`, a = C, u2 = w;
          }
        }
      }
      return n;
    ...
    function K(l) {
      if (!M2.value || !o.value || !A.value) return;
      const { x: e, y: n } = V(l);
      P.value += ` ${e} ${n}`, A.value.setAttribute("d", P.value);
    }
    function B(l) {
      if (M2.value && o.value && A.value) {
        const { x: e, y: n } = V(l);
    ...
    function ne() {
      !s.svgRef || !s.active || (h.value === "draw" ? (s.svgRef.addEventListener("mousedown", I), s.svgRef.addEventListener("mousemove", K), s.svgRef.addEventListener("mouseup", B), s.svgRef.addEventListener("mouseleave", B), s.svgRef.addEventListener("touchstart", I, { passive: false }), s.svgRef.addEventListener("touchmove", K, { passive: false }), s.svgRef.addEventListener("touchend", B)) : h.value === "text" && s.svgRef.addEventListener("mousedown", O), o.value && (o.value.style.pointerEvents = "auto"));
    }
    function U() {
    ...
    return watch(() => s.active, (l) => {
    ...
    }), (l, e) => d.active ? (openBlock(), createElementBlock("div", {
      key: 0,
      "data-dom-to-png-ignore": "",
      class: "vue-ui-pen-and-paper-actions",
      style: normalizeStyle({ backgroundColor: d.backgroundColor })
    }, [
      ...
      createBaseVNode("button", {
        class: normalizeClass(["vue-ui-pen-and-paper-action", { "vue-ui-pen-and-paper-action-active": h.value === "text" }]),
      ...
      createBaseVNode("button", {
        class: normalizeClass(["vue-ui-pen-and-paper-action", { "vue-ui-pen-and-paper-action-disabled": !b.value.length }]),
      ...
      createBaseVNode("button", {
        class: normalizeClass(["vue-ui-pen-and-paper-action", { "vue-ui-pen-and-paper-action-disabled": !E.value.length }]),
        onClick: de,
        style: normalizeStyle({
          backgroundColor: d.backgroundColor,
          border: `1px solid ${y.value}`
      ...
      h.value === "draw" ? withDirectives((openBlock(), createElementBlock("input", {
        key: 0,
        ref: "range",
        type: "range",
        ...
        style: normalizeStyle({ accentColor: d.color })
      }, null, 4)), [
        [vModelText, $.value]
      ]) : createCommentVNode("", true),
      h.value === "text" ? withDirectives((openBlock(), createElementBlock("input", {
        key: 1,
        ref: "range",
        type: "range",
        class: "vertical-range",
...
//# sourceMappingURL=PenAndPaper-7BWf7zgA-KPVBEQ5D.js.map
