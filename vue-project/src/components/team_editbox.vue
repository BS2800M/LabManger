...
<div id="editbox" :style="uiState.editboxstyle1" >
      ...
      <div id="content2">
        <p>其他说明</p>  
        <el-input v-model="formData.location" style="width: 300px" placeholder="" />
      </div>
      <div id="editboxbutton">
        <el-button class="button" size="large" type="warning" :style="uiState.editbuttonhide" @click="modify_Team"  :disabled="uiState.editbox_disablebutton" >修改</el-button>
        <el-button class="button" size="large" type="success" :style="uiState.addbuttonhide" @click="add_Team"  :disabled="uiState.editbox_disablebutton">增加</el-button>
...
import { api_modify_Team,api_add_Team } from '@/api/test_manger.js'
...
function modify_Team() {
  const { name, phone,long_info} = formData
  api_modify_Team(formData.seleteid, name, phone,long_info)
    .then(data => {
      closeeditbox()
      // 触发模板更新事件，通知父组件刷新列表
...
function add_Team() {
  const { name, phone,long_info} = formData
  api_add_Team(name, phone,long_info)
    .then(data => {
      closeeditbox()
      // 触发模板更新事件，通知父组件刷新列表
      eventBus.emit(EVENT_TYPES.TEAM_UPDATED)
