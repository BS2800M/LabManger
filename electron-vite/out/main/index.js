"use strict";
const { app, BrowserWindow, ipcMain } = require("electron");
const path = require("path");
const fs = require("fs");
var mainWindow = null;
var printWindow = null;
async function read_conf() {
  try {
    let data = fs.readFileSync("./conf.txt", "utf8").toString();
    data = JSON.parse(data);
    let printerlist = await mainWindow.webContents.getPrintersAsync();
    data.printerlist = printerlist;
    return data;
  } catch (error) {
    console.log(error);
    return null;
  }
}
async function saveconf(in_printername, in_select_printerid, in_allowprint, in_scale) {
  let data = await read_conf();
  let writedata = {};
  writedata.baseurl = data.baseurl;
  writedata.printername = in_printername;
  writedata.select_printerid = in_select_printerid;
  writedata.allow_print = in_allowprint;
  writedata.scale = in_scale;
  writedata = JSON.stringify(writedata);
  fs.writeFileSync("./conf.txt", writedata, "utf8");
}
async function gotoprint(inputlistdata) {
  let printer = await read_conf();
  printOptions = {
    deviceName: printer.printername,
    silent: true
  };
  printWindow.webContents.send("printdata_send", inputlistdata);
}
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1200,
    minWidth: 1200,
    height: 800,
    minHeight: 800,
    webPreferences: {
      preload: path.join(__dirname, "../preload/index.js")
    }
  });
  printWindow = new BrowserWindow({
    show: false,
    //隐藏窗口
    width: 500,
    height: 300,
    frame: true,
    // 禁用窗口框架，移除菜单和标题栏
    contextIsolation: false,
    enableRemoteModule: true,
    nodeIntegration: true,
    webPreferences: {
      preload: path.join(__dirname, "../preload/index.js")
    }
  });
  ipcMain.handle("read_conf", read_conf);
  ipcMain.handle("save_printer", async (event, args1, args2, args3, args4) => {
    saveconf(args1, args2, args3, args4);
  });
  ipcMain.handle("gotoprint", async (event, args) => {
    gotoprint(args);
  });
  ipcMain.on("print", () => {
    printWindow.webContents.print(printOptions);
  });
  const isDev = process.env.NODE_ENV === "development";
  if (isDev) {
    mainWindow.webContents.openDevTools();
    printWindow.webContents.openDevTools();
    mainWindow.loadURL("http://localhost:5173");
    printWindow.loadURL("http://localhost:5173/#/print");
    app.commandLine.appendSwitch("ignore-certificate-errors");
  } else {
    mainWindow.setMenu(null);
    mainWindow.webContents.openDevTools();
    printWindow.loadFile("./out/renderer/index.html", { hash: "print" });
    mainWindow.loadFile("./out/renderer/index.html");
  }
}
app.whenReady().then(() => {
  createWindow();
  app.on("activate", function() {
    if (BrowserWindow.getAllWindows().length === 0) createWindow();
  });
  mainWindow.on("close", () => {
    app.quit();
  });
});
app.on("window-all-closed", function() {
  if (process.platform !== "darwin") app.quit();
});
